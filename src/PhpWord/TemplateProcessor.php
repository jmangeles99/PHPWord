...
class TemplateProcessor
{
    ...
    public function cloneBlock($blockname, $clones = 1, $replace = true, $indexVariables = false, $variableReplacements = null)
    {
        ...
        $xml = mb_substr($this->tempDocumentMainPart, $idx_start, $idx_end - $idx_start);

        // --- //

        $idx_content_start =   mb_strpos($xml, 'p>');
        $idx_content_end   =  mb_strrpos($xml, '<w:p ');
        ...
        $xmlBlock = mb_substr($xml, $idx_content_start, $idx_content_end - $idx_content_start);
        ...
        if ( $replace )
        {
            ...
            if ( $indexVariables )
                $arrValue = $this->indexClonedVariables($clones, $xmlBlock);
            elseif ( $variableReplacements !== null && is_array($variableReplacements) )
                $arrValue = $this->replaceClonedVariables($variableReplacements, $xmlBlock);
            else
                for ( $i = 1 ; $i <= $clones ; $i++ )
                    $arrValue[] = $xmlBlock;
            ...
            $this->tempDocumentMainPart = str_replace($xml, $arrValue, $this->tempDocumentMainPart);
        }
